...
banco.exec(`
  CREATE TABLE IF NOT EXISTS usuarios (
    ...
    role            TEXT    DEFAULT 'user'
                CHECK(role IN ('user','nutricionista','admin')),
  ...
  CREATE TABLE IF NOT EXISTS planos_receitas (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    plano_id        INTEGER NOT NULL,
    receita_id      INTEGER NOT NULL,
    dia_plano       INTEGER,
    refeicao        TEXT CHECK(refeicao IN ('café','lanche','almoço','lanche2','jantar')),
  ...
  CREATE TABLE IF NOT EXISTS planos_exercicios (
    ...
    dia_semana      INTEGER,
    ordem           INTEGER,
    series_personalizado INTEGER,
    repeticoes_personalizado INTEGER,
    FOREIGN KEY (plano_id) REFERENCES planos_alimentares(id),
    FOREIGN KEY (exercicio_id) REFERENCES exercicios(id)
  );

  CREATE TABLE IF NOT EXISTS usuarios_seguem_planos (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    saiu_da_dieta   INTEGER DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS bloqueios (
      ...
      motivo          TEXT,
      data_bloqueio   TEXT    DEFAULT (datetime('now','localtime')),
  ...
  CREATE TABLE IF NOT EXISTS analytics_diaria (
      id              INTEGER PRIMARY KEY AUTOINCREMENT,
      data            DATE    UNIQUE,
      total_usuarios  INTEGER,
      usuarios_ativos INTEGER,
      nutricionistas_verificados INTEGER,
      receitas_publicadas INTEGER,
      exercicios_publicados INTEGER,
      planos_ativos INTEGER,
      total_comentarios INTEGER,
      total_mensagens INTEGER,
      usuarios_novos INTEGER,
      churn_rate      REAL,
      tempo_sessao_media REAL,
      peso_medio      REAL,
      data_criacao    TEXT    DEFAULT (datetime('now','localtime'))
  );
`)
